...
myIndexedDB.init = function initDB(func) {
  // 首先检测浏览器indexedDB可用性
  if (!window.indexedDB) {
  ...
  request.onsuccess = function success(event) {
    ...
    myIndexedDB.userId = myIndexedDB.getId();
    func();
  };
...
// 获取当前的ID值
myIndexedDB.getId = function getId() {
...
// Create 增加
// 添加一个数据到数据库中
myIndexedDB.createOneDataToDB = function createOneDataToDB(newData) {
  // 添加list数据到数据库中
  var transaction = myIndexedDB.db.transaction(['user'], 'readwrite');
  var storeHander = transaction.objectStore('user');
...
myIndexedDB.retrieveOneDataFromDB = function retrieveOneDataFromDB(index, func, that) {
...
myIndexedDB.retrieveDataWhetherDoneFromDB = function retrieveDataWhetherDoneFromDB(whether, func) {
...
// 获取数据库的所有数据
myIndexedDB.retrieveAllDataFromDB = function retrieveAllDataFromDB(refresh) {
  var transaction = myIndexedDB.db.transaction(['user'], 'readwrite');
  var storeHander = transaction.objectStore('user');
  var range = IDBKeyRange.lowerBound(0, true);
  var allDataArr = [];
...
// Update: 更新
// 更新一个数据同步到数据库中
myIndexedDB.updateDateInDB = function updateDateInDB(changedData) {
  var transaction;
  var storeHander;
...
// 删除数据库中的一个数据
myIndexedDB.deleteOneDataInDB = function deleteOneDataInDB(dataId) {
  var transaction = myIndexedDB.db.transaction(['user'], 'readwrite');
  var storeHander = transaction.objectStore('user');
...
// 删除数据库中的所有数据
myIndexedDB.deleteAllDataInDB = function deleteAllDataInDB() {
  var transaction = myIndexedDB.db.transaction(['user'], 'readwrite');
  var storeHander = transaction.objectStore('user');
  var range = IDBKeyRange.lowerBound(0, true);
